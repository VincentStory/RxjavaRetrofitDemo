...
public class MThreadRunner implements Callback, IThreadRunner {
    ...
    private static final int DEFAULT_MAX_THREADNUM = 3;

    public static int MAX_THREAD_NUM = DEFAULT_MAX_THREADNUM;
    ...
    protected MThreadRunner() {
        ...
        mThreadPool = Executors.newScheduledThreadPool(coreNum * getMaxThreadNum(), new ThreadFactory() {
    ...
    public synchronized int getNumCores() {
        if (coresNum == 0) {
     ...
     * @param group    任务分组
     */
    public synchronized void runTask(Callable<?> callable, Callback callback, String group, ThreadPriority priority) {
        if (callable == null)
            return;
        ...
        synchronized (this) {
            ...
            while (interator.hasNext()) {
                ...
                if (TextUtils.isEmpty(toRun.mGroupName)) {
                    ...
                    if (groupinfo == null) {
                        groupinfo = new GroupInfo();
                        Log.i("","first task belongs to a group has not been excute Immediately");
